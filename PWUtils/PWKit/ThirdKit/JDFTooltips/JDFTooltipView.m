...
- (void)showAtPoint:(CGPoint)point inView:(UIView *)view width:(CGFloat)width arrowDirection:(JDFTooltipViewArrowDirection)arrowDirection animation:(BOOL)animation
{
    self.arrowDirection = arrowDirection;
    ...
    CGRect tooltipFrame = [self tooltipFrameForArrowPoint:point
                                                    width:(self.tooltipTextLabel.frame.size.width + [self arrowHeight] + [self labelPadding])//
                                               labelFrame:labelFrame
                                           arrowDirection:self.arrowDirection
                                             hostViewSize:self.superview.frame.size];
    ...
    float time = animation ? 0.3 : 0;
    // Perform the animation
    [UIView animateWithDuration:time delay:0.0 usingSpringWithDamping:0.6f initialSpringVelocity:0.0f options:UIViewAnimationOptionCurveEaseInOut animations:^{
...
- (void)show
{
    [self showAtPoint:self.arrowPoint inView:self.tooltipSuperview width:self.width arrowDirection:self.arrowDirection animation:YES];
}

- (void)showInView:(UIView *)view
{
    [self showAtPoint:self.arrowPoint inView:view width:self.width arrowDirection:self.arrowDirection animation:YES];
}
...
- (CGRect)tooltipFrameForArrowPoint:(CGPoint)point width:(CGFloat)width labelFrame:(CGRect)labelFrame arrowDirection:(JDFTooltipViewArrowDirection)arrowDirection hostViewSize:(CGSize)hostViewSize
{
    ...
    tooltipFrame.size.height = self.tooltipTextLabel.frame.size.height + 20 + [self arrowHeight]; //[self labelPadding]
...
- (void)drawCanvas1WithFrame:(CGRect)frame;
{
    ...
    {
        //// Rectangle Drawing
        CGRect rect = CGRectMake(CGRectGetMinX(frame) + 7, CGRectGetMinY(frame) + 7, CGRectGetWidth(frame) - 14, CGRectGetHeight(frame) - 14);
