...
- (id)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        ...
        UIImageView *stateIndicator = [[UIImageView alloc] init];
         NSBundle *bundle = [NSBundle bundleWithPath:[[[NSBundle mainBundle] resourcePath] stringByAppendingPathComponent:@"SDRefresh.bundle"]];
        stateIndicator.image = [UIImage imageNamed:@"sdRefeshView_arrow.png" inBundle:bundle compatibleWithTraitCollection:nil];
...
//        textIndicator.bounds = CGRectMake(0, 0, self.frame.size.width, 30);
//        textIndicator.adjustsFontSizeToFitWidth = YES;
//        textIndicator.minimumScaleFactor = .5;
...
//        timeIndicator.bounds = CGRectMake(0, 0, self.frame.size.width, 16);
//        timeIndicator.adjustsFontSizeToFitWidth = YES;
//        timeIndicator.minimumScaleFactor = .5;
...
- (void)hiddenText{
    _textIndicator.hidden = YES;
    _timeIndicator.hidden = YES;
}

- (void)showText{
    _textIndicator.hidden = NO;
